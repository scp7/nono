...
pub fn load_builtin_profile(name: &str) -> Option<Profile> {
    // Map of profile names to their embedded content
    // These are generated by build.rs from data/profiles/*.toml
    let content = match name {
        "claude-code" => Some(include_str!(concat!(
            env!("OUT_DIR"),
            "/profile-claude-code.toml"
        ))),
        "openclaw" => Some(include_str!(concat!(
            env!("OUT_DIR"),
            "/profile-openclaw.toml"
        ))),
        "opencode" => Some(include_str!(concat!(
            env!("OUT_DIR"),
            "/profile-opencode.toml"
        ))),
        "cargo-build" => Some(include_str!(concat!(
            env!("OUT_DIR"),
            "/profile-cargo-build.toml"
        ))),
        _ => None,
    }?;
