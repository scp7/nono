...
- **Read+write access** to the current working directory (`$WORKDIR`)
...
The built-in profile grants access to `$WORKDIR` (wherever you run the command). To limit access to a specific directory regardless of where you invoke it:
