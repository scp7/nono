...
nono includes a built-in `openclaw` profile that you can use immediately without any configuration:
...
If you need different permissions, you can create a custom profile. Custom profiles are stored in:

```
~/.config/nono/profiles/<name>.toml
```

For example, to create a stricter openclaw profile with read-only config access:

```bash
mkdir -p ~/.config/nono/profiles
```

Then create `~/.config/nono/profiles/openclaw-strict.toml`:
...
```bash
# Profile includes secrets, so just pass --secrets to enable loading
...
```bash
# Run the gateway with secrets injected as environment variables
